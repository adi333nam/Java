...
import com.cognizant.cp.dto.ErrorResponseDto;
import com.cognizant.cp.exception.ComponentTypeNotFoundException;
import com.cognizant.cp.exception.RequestNotFoundException;
...
public class ComponentProcessingErrorController {
	...
	public ErrorResponseDto handleComponentTypeException(Exception ex, HttpServletRequest request) {
		return new ErrorResponseDto(new Date(), HttpStatus.NOT_FOUND.value(), HttpStatus.NOT_FOUND.getReasonPhrase(), ex.getMessage(), request.getRequestURI());
	}
